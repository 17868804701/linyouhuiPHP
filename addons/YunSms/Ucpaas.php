...
class Ucpaas
{
    ...
    private function connection($url, $body, $type,$method)
    {
        ...
        if (function_exists("curl_init")) {
            ...
            $result = curl_exec($ch);
            curl_close($ch);
            //trace($result."keywordID".$header['Authorization:']."cityCode".$ch.'hotel'.$body,'有园sss888','DEBUG',true);
        } else {
            $opts = array();
            $opts['http'] = array();
            $headers = array(
            ...
            trace($result."keywordID". $headers['header'][ 'Authorization:']."cityCode". $headers['header'][ 'Content-Length:'].'hotel'.$body,'有园sss999','DEBUG',true);
        }
        return $result;
    }
