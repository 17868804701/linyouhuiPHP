...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function createMenu($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		trace($this->access_token,'access_token');
	...
	public function getOauthAccessToken(){
        ...
        trace('getOauthAccessToken','info');
        trace($code,'info');
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
                trace($this->errCode,'errCode');
                trace($this->errMsg,'errCode');
				return false;
			}
            trace($json,'info');
     ...
     * }
     */
    public function getWxappSession($js_code,$appid='',$secret=''){
        if (!$appid || !$secret) {
            $appid = $this->appid;
            $secret = $this->appsecret;
        }
        if (!$js_code) return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::WXAPP_SESSION_URL.'appid='.$appid.'&secret='.$secret.'&js_code='.$js_code.'&grant_type=authorization_code');
        $url = self::API_BASE_URL_PREFIX.self::WXAPP_SESSION_URL.'appid='.$appid.'&secret='.$secret.'&js_code='.$js_code.'&grant_type=authorization_code';
        trace($url,'URL');
        trace($result,'getWxappSessioninfo1');
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                ...
                $this->errMsg = $json['errmsg'];
                trace($this->errCode,'errCode');
                trace($this->errMsg,'errMsg');
                return false;
            }
            trace($json,'getWxappSessionjson2');
            return $json;
        }
        return false;
    }
