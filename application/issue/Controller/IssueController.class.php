...
class IssueController extends AdminController
{
    ...
    public function operate($type = 'move', $from = 0)
    {
        ...
        if ($type === 'move') {
            ...
            $builder->title(lang('_CATEGORY_MOVE_'))->keyId()->keySelect('pid',lang('_FATHER_CLASS_'), lang('_FATHER_CLASS_SELECT_'), $opt)->buttonSubmit(U('Issue/add'))->buttonBack()->data($from)->display();
        } else {

            $builder->title(lang('_CATEGORY_COMBINE_'))->keyId()->keySelect('toid', lang('_CATEGORY_T_COMBINE_'), lang('_CATEGORY_T_COMBINE_SELECT_'), $opt)->buttonSubmit(U('Issue/doMerge'))->buttonBack()->data($from)->display();
        }
    ...
    public function doMerge($id, $toid)
    {
        ...
        $this->success(lang('_SUCCESS_CATEGORY_COMBINE_') . $effect_count . lang('_CONTENT_GE_'), U('issue'));
        //TODO 实现合并功能 issue
    }
    ...
    public function contents($page = 1, $r = 10)
    {
        ...
        $builder->title(lang('_CONTENT_MANAGE_'))
            ->setStatusUrl(U('setIssueContentStatus'))->buttonDisable('', lang('_AUDIT_UNSUCCESS_'))->buttonDelete()
    ...
    public function verify($page = 1, $r = 10)
    {
        ...
        $builder->title(lang('_CONTENT_AUDIT_'))
            ->setStatusUrl(U('setIssueContentStatus'))->buttonEnable('', lang('_AUDIT_SUCCESS_'))->buttonDelete()
            ->keyId()->keyLink('title', lang('_TITLE_'), 'Issue/Index/issueContentDetail?id=###')->keyUid()->keyCreateTime()->keyStatus()
            ->data($list)
            ->pagination($totalCount, $r)
            ->display();
    ...
    public function setIssueContentStatus()
    {
        ...
        if ($status == 1) {
            foreach ($ids as $id) {
                ...
                D('Common/Message')->sendMessage($content['uid'],$title = lang('_MESSAGE_AUDIT_ISSUE_CONTENT_'), lang('_MESSAGE_AUDIT_ISSUE_CONTENT_VICE_'),  'Issue/Index/issueContentDetail', array('id' => $id), is_login(), 2);
                /*同步微博*/
    ...
    public function contentTrash($page = 1, $r = 10, $model = '')
    {
        ...
        $builder->title(lang('_CONTENT_TRASH_'))
