...
class IndexController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $sub_menu =
            array(
                'left' =>
                    array(
                        array('tab' => 'home', 'title' =>lang('_HOME_'), 'href' => U('Issue/index/index')),
                    ),
            );
        if (check_auth('addIssueContent')) {
            $sub_menu['right'] = array(
                array('tab' => 'post', 'title' => lang('_RELEASE_'), 'href' => '#frm-post-popup','a_class'=>'open-popup-link')
            );
        }
        foreach ($tree as $cat) {
            if ($cat['_']) {
                $children = array();
                $children[] = array('tab' => 'cat_' . $cat['id'], 'title' => lang('_ALL_'), 'href' => U('Issue/index/index', array('issue_id' => $cat['id'])));
        ...
        $sub_menu['first']=array('title'=>lang('_MODULE_'));
    ...
    {
        if (!check_auth('addIssueContent')) {
            $this->error(lang('_AUTHORITY_LACK_'));
        }
        $issue_id = intval($issue_id);
        if (!is_login()) {
            $this->error(lang('_FIRST_LOGIN_'));
        }
        if (!$cover_id) {
            $this->error(lang('_NEED_COVER_'));
        }
        if (trim(op_t($title)) == '') {
            $this->error(lang('_NEED_TITLE_'));
        }
        if (trim(op_h($content)) == '') {
            $this->error(lang('_NEED_CONTENT_'));
        }
        if ($issue_id == 0) {
            $this->error(lang('_NEED_CATEGORY_'));
        }
        if (trim(op_h($url)) == '') {
            $this->error(lang('_NEED_WEBSITE_'));
        }
        ...
        if ($id) {
            ...
            if (!check_auth('editIssueContent')) { //不是管理员则进行检测
                if ($content_temp['uid'] != is_login()) {
                    $this->error(lang('_FORBID_TO_OTHER_'));
                }
            }
            $content['uid'] = $content_temp['uid']; //权限矫正，防止被改为管理员
            $rs = D('IssueContent')->save($content);
            if ($rs) {
                $this->success(lang('_SUCCESS_EDIT_'), U('issueContentDetail', array('id' => $content['id'])));
            } else {
                $this->success(lang('_FAIL_EDIT_'), '');
            }
        } else {
            if (modC('NEED_VERIFY', 0) && !is_administrator()) //需要审核且不是管理员
            {
                $content['status'] = 0;
                $tip = lang('_TIP_AUDIT_');
                $user = query_user(array('nickname'), is_login());
                $admin_uids = explode(',', C('USER_ADMINISTRATOR'));
                foreach ($admin_uids as $admin_uid) {
                    D('Common/Message')->sendMessage($admin_uid, $title = lang('_WARN_CONTRIBUTE_'),"{$user['nickname']}".lang('_PLEASE_AUDIT_'),  'Admin/Issue/verify', array(),is_login(), 2);
                }
            }
            $rs = D('IssueContent')->add($content);
            if ($rs) {
                $this->success(lang('_SUCCESS_CONTRIBUTE_') . $tip, 'refresh');
            } else {
                $this->success(lang('_FAIL_CONTRIBUTE_'), '');
            }
        }
    ...
    public function issueContentDetail($id = 0)
    {
        ...
        $this->setTitle('{$content.title|op_t}' . '——'.lang('_MODULE_'));
