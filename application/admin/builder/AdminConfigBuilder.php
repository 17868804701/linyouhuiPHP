...
class AdminConfigBuilder extends AdminBuilder
{
    ...
    //关联数组选项
    public function keySelectArr($name, $title, $subtitle = null, $options)
    {
        return $this->key($name, $title, $subtitle, 'selectArr', $options);
    }
    public function keyCatSelect($name, $title, $subtitle = null, $options)
    {
        $map = array();
    ...
    public function handleConfig()
    {
        ...
        if (request()->isPost()) {
            ...
            foreach (input('') as $k => $v) {
                ...
                $configData = $configModel->where(['name' => $name])->find();  //根据名字，表的unique key查询数据
                trace($configData,'数据');
                trace($name,'提交了什么数据2');
                $data['name'] = $name;
                $data['type'] = 0;
