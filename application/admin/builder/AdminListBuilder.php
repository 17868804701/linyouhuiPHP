...
//        $this->_buttonList['title'] = $title;//
//        $this->_buttonList['attr'] = $attr;
        return $this;
    }
    ...
    {
        return $this->button(lang('_CLEAR_OUT_'), array('class' => 'btn btn-default ajax-post tox-confirm', 'data-confirm' => lang('_CONFIRM_CLEAR_OUT_'), 'url' => url('', array('model' => $model)), 'target-form' => 'ids', 'hide-data' => 'true'));
    }
...
//            $this->setSearchPostUrl('');
            ...
            if (empty($arrdb)) {
                $this->_search[] = array('title' => $title, 'name' => $name, 'type' => $type, 'des' => $des, 'attr' => $attr, 'arrvalue' => $arrvalue);
                $this->setSearchPostUrl('');
            } else {
    ...
    {
        $map = array(-1 => lang('_DELETE_'), 1 => lang('_DISABLE_'), 0 => lang('_ENABLE_'), 2 => lang('_UNAUDITED_'));
    ...
    public function keyOwner($name, $title,$attr=array())
    {
        return $this->key($name, $title, 'owner',$attr);
    }

    public function keyBer($name, $title,$attr=array())
    {
        return $this->key($name, $title, 'buyer',$attr);
    }
    //店铺
    public function keyShop($name, $title,$attr=array())
    {
        return $this->key($name, $title, 'shop',$attr);
    }
    public function keyWechat($name,$title,$attr = array())
    {
        return $this->key($name, $title, 'wechat',$attr);
    }
     ...
     * @throws DbException
     */
    public function paginates($listRows = null, $simple = false, $config = [])
    {
        $config = [
            'type'     => 'bootstrap',
            'var_page' => 'page',
        ...
        $listRows = $listRows ? $listRows : $config['list_rows'];
        ...
        return $class::make($this->_data, $listRows, $page, $total, $simple, $config);
    }

    /**
     * $solist 判断是否属于选择返回数据的列表页，如果是在列表页->fetch('admin_solist');@mingyangliu
     * @author Patrick <contact@uctoo.com>
     * */
    protected function fetch($templateFile = '', $vars = [], $replace = [], $config = [] )
    {
        ...
        $this->convertKey('image', 'html', function ($value, $key, $item) {
            ...
            if (intval($value)) {//value是图片id
                ...
                $html = "<div class='popup-gallery'><a title=\"" . lang('_VIEW_BIGGER_') . "\" href=\"$sc_src\"><img src=\"$sc_src\"/ ".$content."></a></div>";
            }
            return $html;
        });
        ...
        $this->convertKey('owner', 'html', function ($value, $key, $item) {
            $content = '';
            if (intval($value)) {//value是图片id
                $user = model('ucuser')->find($value);
            }
            return $user['name'] .'-'.$user['wechat'].'-'.$user['mobile'];
//            return $user['wechat'];
        });
        //输出买家

                $this->convertKey('buyer', 'html', function ($value, $key, $item) {
                    $content = '';
                    if (intval($value)) {//value是图片id
                        $user = model('ucuser')->find($value);
                    }
        ...
        //输出微信
        $this->convertKey('wechat', 'html', function ($value, $key, $item) {
            $content = '';
            $where = explode(':',$value);
            if (count($where) != 0) {//value是图片id
                $user = model('ucuser')->where(array('name'=>$where[0],'wechat'=>$where[1]))->find();
            }
            return $user['nickname'];
        });

        //输出店铺
        $this->convertKey('shop', 'html', function ($value, $key, $item) {
            $content = '';
            if (!intval($value)) {//value是图片id
                $value = intval($value);
            }
            $user = model('shop')->find($value);
...
//            print_r($button);
        ...
        //生成翻页HTML代码  TODO::升级成TP5的分页实现，去掉TP3的分页类
...
//        $paginationHtml = $pager->show();
        $paginationHtml = $this->paginates($this->_pagination['listRows'] , $this->_pagination['totalCount'] );
        ...
        $this->assign('pagination', $paginationHtml->render());
        $this->assign('list', $paginationHtml);
        /*加入搜索 陈一枭*/
        ...
        if ($solist) {
            return view('admin@builder/admin_solist',$vars,$replace,$config);
        } else {
            return view('admin@builder/admin_list',$vars,$replace,$config);
        }
    }
    ...
    public function doSetStatus($model, $ids, $status = 1)
    {
        ...
        $rs = db($model)->where('id','in', $id)->update(array('status' => $status));
        if ($rs === false) {
...
//        $attr['class']='btn btn-ajax btn-success';
    ...
    public function doDeleteTrue($model, $ids)
    {
        ...
        db($model)->where(array('id' => array('in', $ids)))->delete();
