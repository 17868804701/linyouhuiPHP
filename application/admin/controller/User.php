...
//        $map['status'] = array('egt', 0);
    ...
    public function expandinfo_select($page = 1, $r = 20)
    {
        ...
        $list = db('Member')->where($map)->order('last_login_time desc')->paginate($page, $r);
        $totalCount = db('Member')->where($map)->count();
        int_to_string($list);
        //扩展信息查询
        ...
        $fields_list = db('field_setting')->where($map_profile)->column('id,field_name,form_type');
        ...
        $builder->setSearchPostUrl(url('Admin/User/expandinfo_select'))->search(lang('_SEARCH_'), 'nickname', 'text', L
        ('_PLACEHOLDER_NICKNAME_ID_'));
    ...
    public function submitNickname()
    {
        ...
        if ($res) {
            ...
            return ['info'=>lang('_MODIFY_NICKNAME_SUCCESS_')];
        } else {
            return ['info'=>lang('_MODIFY_NICKNAME_FAILURE_')];
        }
        }
    }
    ...
    public function submitPassword()
    {
        ...
        if (!$res) {
            return ['info'=>lang('_CHANGE_PASSWORD_FAIL_')];
        } else {
            return ['info'=>lang('_CHANGE_PASSWORD_SUCCESS_')];
        }
    }
    ...
    public function scoreList()
    {
        ...
        $builder
            ...
            ->setStatusUrl(U('setTypeStatus'))->buttonEnable()->buttonDisable()->button(lang('_DELETE_'), array('class' => 'btn ajax-post tox-confirm', 'data-confirm' => '您确实要删除积分分类吗？（删除后对应的积分将会清空，不可恢复，请谨慎删除！）', 'url' => U('delType'), 'target-form' => 'ids'))
            ->button(lang('_RECHARGE_'), array('href' => U('recharge')))
    ...
    public function recharge()
    {
        ...
        if (IS_POST) {
            ...
            foreach ($scoreTypes as $v) {
                ...
                D('Ucenter/Score')->setUserScore($aUids, $aValue, $v['id'], $aAction, '', 0, lang('_BACKGROUND_ADMINISTRATOR_RECHARGE_PAGE_RECHARGE_'));
