...
//	);
	...
	public function add_or_edit_delivery($delivery)
	{
		...
		{
			$ret = $this->allowField(true)->save($delivery);
		}
		else
		{
			$ret = $this->allowField(true)->where('id='.$delivery['id'])->save($delivery);
		}
		return $ret;
	}
	...
	{
		$ret['list'] = db('ShopDelivery')->order('create_time')->select();
		$ret['count'] = db('ShopDelivery')->count();
		//获取父级分类信息
//		if(!empty($ret['list'])) {
