...
class ShopOrder extends Model {
	...
	const ORDER_WAIT_USER_PAY     = 0; //待付款
	const ORDER_WAIT_USER_PAY_OK    = 1; //y付款
...
//	);
	...
	public function get_order_list($option)
	{
		...
		if (!empty($option['pay_status']))
		{
			$where_arr[] = 'pay_status = ' . $option['pay_status'];
		}
		if (!empty($option['no_order_status']))
		{
			$where_arr[] = 'order_status != ' . $option['no_order_status'];//不是这个状态的
		}
		...
		$ret['list']  = db('shop_order')->where($where_str)->limit(($option['page']-1)*$option['r'],$option['r'])->order('add_time desc')->select();
		$ret['count'] = db('shop_order')->where($where_str)->count();
		return $ret;
	}
	...
	public function get_order_by_id($id)
	{
        ...
        $ret['goods'] = db('OrderGoods')->where("order_id",$id)->select();
		return $ret;
	}
...
//			self::ORDER_WAIT_SHOP_ACCEPT=> '待接单',
            self::ORDER_WAIT_USER_PAY_OK => '已付款',
