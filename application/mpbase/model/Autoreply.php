    ...
    public function post_messages($data){
        ...
        switch($data['type']){
            case 'picture':
                ...
                trace($data,'插入');
//                !$data['mp_id']?
//                $res = db('picture_messages')->field('title0,detile0,url0,title1,detile1,url1,title2,detile2,url2,title3,detile3,url3,title4,detile4,url4,pic')->insert($data);
                $res = db('picture_messages')->insertGetId($data);
                break;
            case 'text':
                $textmodel = db('text_messages');
//                $detile['detile'] = $data['detile'];
//                !$data['mp_id']?
                $res = $textmodel->field('detile')->insertGetId($data);
