...
class Messages extends Model
{
    ...
    public function get_replay_all($where = null, $r = 10, $page = 1)
    {
        ...
        foreach ($select['data'] as &$v) {
            switch ($v['type']) {
                case'text':
                    $v['editaction'] = '<a href="' . url('edit_text_messages', array('id' => $v['id'])) . '">查看or编辑</a>';
                    break;
                case'picture':
                    $v['editaction'] = '<a href="' . url('edit_picture_messages', array('id' => $v['id'])) . '">查看or编辑</a>';
                    break;
            }
            ...
            switch ($v['mtype']) {
                case 1 :
                    ...
                    $v['statu'] = '<a href="' . url($statu['url'], array('id' => $v['id'])) . '">' . $statu['text'] .
                        '</a>';
                    break;
                case 2 :
                    $v['mtype'] = '消息自动回复';
...
//        $data = $params['weObj']->getRevData();
        ...
        switch($data['type']){
            case 'text':
                $rep_data = db('text_messages')->where($ms_id)->find();
