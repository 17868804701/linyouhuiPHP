...
class Mpbase extends Admin
{
    ...
    public function aredit($id = null)
    {
        ...
        if (IS_POST) {   //提交表单
            ...
            $data['mp_id'] = input('post.mp_id', '', 'op_t');
            $data['type'] = input('post.type', '', 'intval');
            $data['keyword_id'] = input('post.keyword_id', '', 'intval');
            ...
            $builder = new AdminConfigBuilder();
                ...
                $builder->title($id != 0 ? '编辑自动回复' : '添加自动回复')
                    ...
                    ->keyStatus()
                    ->data($ar)
                    ->buttonSubmit(url('aredit?id=###'))->buttonBack()
                    ->display();
        }
    ...
    public function index($page=1,$r=20)
    {
        ...
        $builder
            ...
            ->setStatusUrl(url('setStatus'))->buttonEnable()->buttonDisable()->button(lang('_DELETE_'),array('class' => 'btn ajax-post tox-confirm', 'data-confirm' => '您确实要删除公众号吗？（删除后对应的公众号配置将会清空，不可恢复，请谨慎删除！）', 'url' => url('del'), 'target-form' => 'ids'))
    ...
    public function edit($id = null)
    {
        ...
        if (request()->isPost()) {   //提交表单
            ...
            if ($id != 0) {  //编辑
                ...
                $mp = array('uid' => UID,'status' => 1,'notify_url' => httpTohttps(url('mpbase/weixin/notify','',false,true)));
            }
            ...
            if(is_administrator()){   //管理员可以设定公众号uid
                $builder->title($id != 0 ? '编辑公众号' : '添加公众号')
                    ...
                    ->keyText('notify_url', '接收微信支付异步通知回调地址', '默认值'.httpTohttps(url('mpbase/weixin/notify','',false,true)))
                    ->keyStatus()
                    ->data($mp)
    ...
    public function change() {
        ...
        session ( 'mp_token' , $info['public_id']);
        session ( 'mp_id', $info['mp_id'] );
        $appinfo = model ( 'MemberPublic' )->where ( 'mp_id', $info['mp_id'])->find ();
    ...
    public function replay_messages($r=10,$page=1){
        $autor = model('Mpbase/Autoreply');
        $messages = model('Mpbase/Messages');
        input('mtype')? $where['mtype'] = input('mtype'):null;
        $where['mp_id']=get_mpid();
//        $list = $messages->get_replay_all($where);
        $list = db('ReplayMessages')->where($where)->select();
//        dump($list);
        ...
        $mtype['mtype'] = input('mtype');

        return $builder
            ->title('自动消息回复')
            ->buttonNew(url('edit_text_messages',$mtype),'新增文本消息')
            ->buttonNew(url('edit_picture_messages',$mtype),'新增图文消息')
            ->setSelectPostUrl(url(''))
            ->select('动作类型','mtype','select','','','',$message_type)
            ->keyId()
            ->keyMap('mtype','动作类型',array(1 => '关注自动回复', 2 => '消息自动回复', 3 => '关键词自动回复'))
            ->keyText('type','回复类型')
            ->keyText('title','规则名称')
            ->keyTime('time','创建时间')
            //转出想要的操作key 为do；
...
//            ->keyDoAction('editaction','操作')
            ->keyDoAction('statu','操作')
            ->keyDoActionEdit('editaction','操作')
            ->data($list)
            ->pagination(count($list),$r)
            ->fetch();
    }

    public function edit_text_messages(){
        $auto = model('Mpbase/Autoreply');
        $model = model('ReplayMessages');
        $id = input('id');
        $list['mtype'] = input('mtype');
...
//            开始事务
            ...
            if(!$id){
                ...
                $res =$model->isUpdate(true)->allowField(true)->save($data,['id'=>$id]);
            }

            if(!$res||!$res_mes){
                $model->rollback();
//                $this->error($res_mes.'错误-'.$res);
                return ['status'=>true,'info'=>$res_mes.'错误-'.$res, url('')];
            }else{
                $model->commit();
//                $this->success('成功',"javascript:history.back(-1);");
                return ['status'=>true,'info'=>'操作成功。', url('mpbase/replay_messages')];
            }
        }else{
        if($id) {
            $list = $model->find($id);
    ...
    public function edit_picture_messages(){
        ...
        $auto = model('Autoreply');
        $model = model('Replay_messages');
        $id = input('id');
        $list['mtype'] = input('mtype');
...
//            开始事务
            ...
            if(isset($id) &&  $id != 0){
                $res =$res =$model->isUpdate(true)->allowField(true)->save($data,['id'=>$id]);
...
//            dump($list);die;
    //        $auto->builder_picture_messages();
            ...
            $builder
                ...
                ->keyText('detile4','内容')
                ->keyText('url4','url4')
                ->keySingleImage('pic','图片','按顺序添加图片')
                ->data($list)
                ->buttonSubmit(url('edit_picture_messages'))
                ->buttonBack();
            return $builder->fetch();
