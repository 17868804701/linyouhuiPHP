...
class InputRenderWidget extends Action
{
    ...
    public function inputRender($data = array(), $type)
    {
        ...
        switch ($data['form_type']) {
            ...
            case 'radio':
                $this->assign('field_name', $data['field_name']);
                $checked = isset($data['field_content']['field_data']) ? $data['field_content']['field_data'] : lang('_SELECT_NOT_');
                if ($type == "personal") {
            ...
            case 'select':
                $this->assign('field_name', $data['field_name']);
                $selected = isset($data['field_content']['field_data']) ? $data['field_content']['field_data'] : lang('_SELECT_NOT_');
                if ($type == "personal") {
                    /*
                     * 添加用户扩展资料可添加关联字段功能（多选支持）
                     * 给原有多选扩展字段做一个join判断
                    ...
                    if($data['child_form_type'] == "join"){
                        ...
                        if ($data['form_default_value'] != '' && $data['form_default_value'] != null) {
                            ...
                            $canSelected[0]['value'] = lang('_NONE_');
