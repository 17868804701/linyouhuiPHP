...
class Member extends Model
{
    ...
    //字段修改器
    public function setRegisteIpAttr($value)
    {
    ...
    public function registerMember($nickname = '')
    {
        /* 在当前应用中注册用户 */
        if ($nickname != '') {
            $user = array('nickname' => $nickname, 'status' => 1);
            $uid = $this->save($user);
            trace($uid,'registerMember save after');
            $uid = $this->uid;
            trace($uid,'registerMember save after111');
            if (!$uid) {
            ...
            if($allFollow != '') {
                ...
                foreach($allFollow as $val) {
                    if(query_user('uid', $val)) {
                        ...
                        db('Member')->where(array('uid' => $val))->setInc('fans', 1);
                    }
                }
            }
            if($allFans != '') {
