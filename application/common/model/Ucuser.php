...
//    }
    ...
    public function registerUser($mp_id = '',$openid = '')
    {
        /* 在当前应用中注册用户 */
        $user = new Ucuser();
        if ($user->save(array('mp_id' => $mp_id,'openid' => $openid, 'status' => 1))) {
            $ucmid = $user->mid;
            if (!$ucmid) {
    ...
    public function login($mid,$mobile = '', $password = '', $remember = false,$role_id=0)
    {
        ...
        if (!empty($user) && $user['status']) {
            /* 验证用户密码 */
            trace(think_ucenter_md5($password, UC_AUTH_KEY),'密码');
            trace($user['password'],'密码2');
     ...
     * autor:uctoo
     */
    public function addSyncUcuser($mid,$uid, $info)
    {
        $data1 = $info;
